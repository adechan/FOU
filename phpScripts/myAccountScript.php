...
require 'database.php';
  ...
  $sql = "SELECT * FROM files WHERE uploaded_by like ? AND location NOT LIKE 'PUBLIC_FILES'";
  ...
  $sql = "SELECT * FROM files WHERE uploaded_by like ? AND location NOT LIKE 'PUBLIC_FILES' LIMIT " . $startRow . "," . $resultsPerPage;
  $stmt = mysqli_stmt_init($connection);
  if(!mysqli_stmt_prepare($stmt,$sql))
  {
    header("Location: index.php?error=sql");
